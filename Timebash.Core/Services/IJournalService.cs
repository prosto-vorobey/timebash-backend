...
public interface IJournalService
{
    ...
    Task<JournalResponse> GetByIdAsync(Guid id, Guid userId, CancellationToken cancellationToken);
    ...
    /// <exception cref="BadRequestException">Thrown when <paramref name="id"/> is empty.</exception>
    /// <exception cref="NotFoundException">Thrown when the journal does not exist or does not belong to the user.</exception>
    Task<ActivitiesListResponse> GetActivitiesByJournalIdAsync(
        Guid id, 
        DateTime? start, 
        DateTime? end, 
        Guid userId, 
    ...
    Task<JournalResponse> CreateAsync(JournalRequest journalRequest, Guid userId, CancellationToken cancellationToken);
    ...
    /// <exception cref="BadRequestException">Thrown when <paramref name="id"/> is empty.</exception>
    /// <exception cref="NotFoundException">Thrown when the journal does not exist or does not belong to the user.</exception>
    Task<bool> UpdateAsync(Guid id, JournalRequest journalRequest, Guid userId, CancellationToken cancellationToken);
    ...
    /// <exception cref="BadRequestException">Thrown when <paramref name="id"/> is empty.</exception>
    /// <exception cref="NotFoundException">Thrown when the journal does not exist or does not belong to the user.</exception>
    Task<ConflictCorrectionsListResponse> GetTimeCorrectionConflictsAsync(
        Guid id, 
        DateTime startTime, 
        DateTime endTime, 
        Guid userId, 
